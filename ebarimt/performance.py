...
def auto_retry_failed_receipts():
    ...
    for receipt in failed_receipts:
        try:
            ...
            log_info("Queued retry for receipt", {"receipt_log": receipt.name, "invoice": receipt.sales_invoice or receipt.pos_invoice})
            retried_count += 1
        except Exception as e:
...
def auto_sync_pending_receipts():
    ...
    for receipt in pending:
        try:
            ...
            log_debug("Queued sync check", {"receipt_log": receipt.name})
            synced_count += 1
        except Exception as e:
...
def auto_void_cancelled_invoices():
    ...
    for receipt in receipts_to_void:
        try:
            ...
            log_info("Queued void for receipt", {"receipt_log": receipt.name, "invoice": receipt.sales_invoice})
            voided_count += 1
        except Exception as e:
