...
"""
eBarimt Telemetry - Anonymous Error Reporting to GitHub Issues

This module provides opt-in error reporting that creates GitHub issues
when critical errors occur. This helps the mn-frappe team improve eBarimt.

Privacy:
- No personal data is collected
- No credentials, TINs, or API keys are sent
...
GITHUB_REPO = "mn-frappe/ebarimt"
GITHUB_API_URL = f"https://api.github.com/repos/{GITHUB_REPO}/issues"

# Rate limiting - max issues per day per site
MAX_ISSUES_PER_DAY = 5
CACHE_KEY = "ebarimt_telemetry_issues_today"


def is_telemetry_enabled() -> bool:
    """Check if telemetry/error reporting is enabled in settings."""
    try:
        return bool(frappe.db.get_single_value("eBarimt Settings", "enable_error_reporting"))
    except Exception:
        return False
...
def get_github_token() -> str | None:
    ...
    try:
        return frappe.db.get_single_value("eBarimt Settings", "github_token")
    except Exception:
        return None
...
def get_environment_info() -> dict:
    ...
    info = {
        ...
        "ebarimt_version": "unknown",
        "python_version": "unknown",
        "environment": "unknown",
    }

    try:
        from ebarimt.hooks import app_version
        info["ebarimt_version"] = app_version
    except Exception:
        pass
    ...
    try:
        info["environment"] = frappe.db.get_single_value("eBarimt Settings", "environment") or "unknown"
    except Exception:
        pass
...
def create_github_issue(
    ...
    if not token:
        frappe.log_error(
            "eBarimt Telemetry: GitHub token not configured",
    ...
    try:
        ...
        frappe.log_error(f"Failed to create GitHub issue: {e}", "eBarimt Telemetry")

    return None


def sanitize_data(data: dict) -> dict:
    """Remove sensitive data like TINs, credentials, etc."""
    sensitive_keys = (
        'password', 'token', 'secret', 'key', 'api_key', 'access_token',
        'tin', 'taxpayer', 'regno', 'merchant_tin', 'operator_tin',
        'pos_no', 'api_username', 'api_password', 'refresh_token'
    )
...
| Property | Value |
|----------|-------|
| eBarimt Version | `{env_info['ebarimt_version']}` |
...
"""
    ...
    body += """---
*This issue was automatically created by eBarimt Telemetry. The user has opted-in to anonymous error reporting.*
*No personal data, TINs, or credentials are included in this report.*
"""
...
def report_error(
    ...
    if not check_rate_limit():
        frappe.log_error(
            "eBarimt Telemetry: Rate limit exceeded for today",
    ...
    if token:
        ...
        if existing:
            frappe.log_error(
                f"eBarimt Telemetry: Similar issue already exists: {existing}",
                "Telemetry Duplicate"
            )
    ...
    if result:
        ...
        frappe.log_error(
            f"eBarimt Telemetry: Created issue {result.get('html_url')}",
            "Telemetry Success"
        )
...
def handle_exception(e: Exception, context: dict | None = None):
    ...
    # Always log locally
    frappe.log_error(stack_trace, f"eBarimt Error: {error_message[:100]}")
...
# API for manual reporting
@frappe.whitelist()
def report_frontend_error(error_message: str, stack_trace: str = "", url: str = ""):
    """
    Report a frontend JavaScript error.

    This endpoint can be called from the frontend to report JS errors.
    """
    if not is_telemetry_enabled():
        return {"status": "disabled"}

    context = {"url": url, "source": "frontend"}
    result = report_error(error_message, stack_trace, context, severity="warning")

    return {"status": "reported" if result else "failed"}
...
def test_telemetry():
    ...
    if not is_telemetry_enabled():
        return {"status": "error", "message": _("Error reporting is not enabled in eBarimt Settings")}
